...
def checkout(version, uname, param):
	...
	db_user = db.app_user.find_one({'uname':uname['uname']}, #, 'mice':{'$ne':1}},
		{'coupon':1, 'address':1, 'credit':1, 'app_id':1, 'mice':1})
	...
	if db_recv:
		...
		if uname['uname'] not in db_recv['unames']: # 补充疑似账号
			db.recv_tel.update_one({'tel':address[2]},{'$push':{'unames':uname['uname']}})
			one_more = 1
		if len(db_recv['unames'])+one_more>10: # 改为10，2015-10-12
			...
			if one_more==1:
				db.app_user.update_one({'uname':uname['uname']},{'$set':{'mice':mice}})
			if mice==1:
				print '!!! mice:', address[2]#, uname['uname'], db_recv['unames']
	...
	if db_recv:
		...
		if uname['uname'] not in db_recv['unames']:
			db.recv_addr.update_one({'addr':address[3]},{'$push':{'unames':uname['uname']}})
			one_more = 1
		if len(db_recv['unames'])+one_more>10: # 改为10，2015-10-12
			# 发现疑似mice，不标注，因为不确定
			print '!!! maybe a mice:', address[3].encode('utf-8') #, uname['uname'], db_recv['unames']
	else:
		db.recv_addr.insert_one({'addr':address[3],'unames':[uname['uname']]})
		#print 'insert', address[2]

	# 用app_id检查黄牛
	appid_count = db.app_user.find({'app_id':app_id},{'_id':1}).count()
	if appid_count>10 and app_id.strip()!='':  # app_id 可能为空，新绑定的用户 # 改为10，2015-10-12
		# 发现 mice
		...
		for m in db_mice:
			if m['uname'] in app_helper.WHITE_LIST: # 过滤白名单
				...
				mice.append(m['uname'])
		db.app_user.update_many({'uname':{'$in':mice}},{'$set':{'mice':1}})
		if mice!=[]:
	...
	for item in cart:
		...
		if r: # 如果库存数据中没此sku，会忽略掉，此情况应该不会发生
			...
			# v3 检查是否买X送Y
			if version=='v3' and item['product_id'] in app_helper.buy_X_give_Y.keys():
	...
	if version=='v2':
		db.order_app.update_one({'order_id':order_id}, {
			'$set'  : order,
			'$push' : {'history'  : (app_helper.time_str(), uname['uname'], '提交结算')}
		}, upsert=True)
	elif version=='v3':
		db.app_user.update_one({'uname':uname['uname']}, {
			'$set'  : { 'cart_order.%s'%param['session'] : order }
		})
...
class handler:        #class PosPosCheckout: 
	def POST(self, version='v2'):
		...
		param = web.input(app_id='', session='', shop_id='', order_id='',
				addr_id='', coupon_id='', use_credit='', cart='', sign='')

		print param

		if '' in (param.app_id, param.session, param.shop_id, 
				param.addr_id, param.cart, param.sign):
		...
		if uname:
			...
			md5_str = app_helper.generate_sign([param.app_id, param.session, param.order_id, 
				param.shop_id, param.addr_id, param.coupon_id, param.use_credit, param.cart])
			if md5_str!=param.sign:
				return json.dumps({'ret' : -1, 'msg' : '签名验证错误'})

			# 记录版本信息
			db.app_user.update_one({'uname':uname['uname']},{'$set':{'version':version}})
