	...
	def POST(self, version='v3'):
		...
		if uname:
			...
			if len(param.order_id)>0:
				...
				if float(ret_json['data']['due'])!=float(db_order.get('due3',db_order['due'])): # 返回的是需第三方支付的金额，所有比对due3，而不是due
				...
				# 可支付
				db.order_app.update_one({'order_id':param.order_id},{
					'$push' : {'history':(app_helper.time_str(), uname['uname'], '提交支付宝支付2')}
				})
				...
				new_order['history']=[(app_helper.time_str(), uname['uname'], '提交支付宝支付')]
