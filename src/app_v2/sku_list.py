#!/usr/bin/env python
# -*- coding: utf-8 -*-

import web, json
from bson.objectid import ObjectId
from config import setting
import app_helper

db = setting.db_web

url = ('/app/(v2|v3)/sku_list')

# 查询商品，允许分段查询返回
class handler:        
	def POST(self, version='v2'):
		web.header('Content-Type', 'application/json')
		if version not in ('v2','v3'):
			return json.dumps({'ret' : -999, 'msg' : '版本错误！'})
		print 'version=',version

		param = web.input(app_id='', shop_id='', category='', page_size='', page_index='', sign='', session='')

		if '' in (param.app_id, param.shop_id, param.category, param.page_size, param.page_index, param.sign):
			return json.dumps({'ret' : -2, 'msg' : '参数错误'})

		if not (param.page_size.isdigit() and param.page_index.isdigit()): 
			return json.dumps({'ret' : -5, 'msg' : 'page参数错误'})

		if len(param.shop_id)<24:
			return json.dumps({'ret' : -2, 'msg' : 'shop_id参数错误'})

		print param
		
		#验证签名
		md5_str = app_helper.generate_sign([param.app_id, param.session, param.shop_id,
			param.category, param.page_size, param.page_index])
		if md5_str!=param.sign:
			return json.dumps({'ret' : -1, 'msg' : '签名验证错误'})

		# 去掉分类号前的'c',兼容ios不能用数字做key
		if param.category[0]=='c':
			category=param.category[1:]
		else:
			category=param.category

		# 处理整箱预售
		if category=='-3': # 关闭整箱预售 2015-10-27
			shop_id = setting.B3_shop # B3整箱预售虚拟店
		else:
			shop_id = param.shop_id

		# 区别首单用户
		if param.session!='':
			uname = app_helper.app_logged(param.session) # 检查session登录
			if uname==None: # uname 可能返回 None, session有问题的时候
				ccc=0
			else:
				ccc = db.order_app.find({'user':{'$in':uname.values()}, 
					'status':{'$nin':['DUE','TIMEOUT','CANCEL']}},{'_id':1}).count()
		else: # 没登录用户，认为是首单
			ccc = 0

		# 查找商品信息清单
		# 分页返回 find().skip(page_size*page_index).limit(page_size)
		condition = {
				'shop'        : ObjectId(shop_id),
				'category'    : category,
				'list_in_app' : {'$ne' : 0},
				'online'      : 1,
				#'num'         : {'$gt' : 0}, # 只显示有库存的商品
		}
		if ccc > 0: # 非首单，要回避只首单可见的商品 2015-09-25
			condition['first_order'] = { '$ne' : 1 }
		'''
		db_invent = db.inventory.find(condition, 
			{
				'product_id'  : 1,
				'category'    : 1, # 品类，从sku_store来的
				'sort_weight' : 1, 
				'sku'         : 1,
				'price'       : 1, # 门店价
				'num'         : 1, # 库存数
			},
			skip  = int(param.page_size)*int(param.page_index), 
			limit = int(param.page_size)
		).sort([('sort_weight',1), ('_id',1)])
		'''

		# 取所有商品数据 2015-10-29
		db_invent = db.inventory.find(condition, 
			{
				'product_id'  : 1,
				'category'    : 1, # 品类，从sku_store来的
				'sort_weight' : 1, 
				'sku'         : 1,
				'price'       : 1, # 门店价
				'num'         : 1, # 库存数
				'hide_after_0' : 1, # 是否售完隐藏
			},
		).sort([('sort_weight',1), ('_id',1)])

		# 库存小于零的沉底 2015-10-29 # 增加过滤售完隐藏
		db_invent2 = []
		db_num_0 = []
		for s in db_invent:
			if int(s['num'])>0:
				db_invent2.append(s)
			else:
				if s.get('hide_after_0', 0)!=1: # 不是售完隐藏商品
					db_num_0.append(s)
		db_invent2.extend(db_num_0)

		# 取指定区间的 2015-10-29
		start_pos = int(param.page_size)*int(param.page_index)
		end_pos = start_pos + int(param.page_size)
		db_invent3 = db_invent2[start_pos:end_pos]

		# 处理商品
		invent = []
		skus = []
		for s in db_invent3:
			skus.append(s['sku'])
			invent.append((
				s['sku'],  #0
				s['price'],    #1
				s['product_id'], #2
				s['num'], #3
			))  

		# 有效的sku
		db_sku=db.sku_store.find({'_id':{'$in':skus}} ,{
			'product_id'    : 1,
			'base_sku'      : 1,
			'app_title'     : 1,
			'is_onsale'     : 1,
			'special_price' : 1,
			'promote'       : 1, 
			'maximun'       : 1,
		}).sort([('_id',1)])

		skus = {}
		for u in db_sku:
			base_sku = db.dereference(u['base_sku'])

			skus[u['_id']]=(
				base_sku['name'],   #0
				base_sku['image'][0] if base_sku.has_key('image') and len(base_sku['image'])>0 else '',
				u['app_title'],   #2
				u['is_onsale'],   #3
				u['special_price'],   #4
				base_sku['original'],   #5
				u['promote'],   #6
				u['maximun'],   #7
			)

		data = []
		for i in invent:
			# 调整价格显示，有分的金额才显示2位小数， 2015-10-30
			if ('%.2f'%float(i[1]))[-1]=='0':
				show_price = '%.1f' % float(i[1])
			else:
				show_price = '%.2f' % float(i[1])

			# 准备数据
			new_one = {
				'product_id' : i[2],
				'title'      : skus[i[0]][2],
				#'original'   : skus[i[0]][5],
				'price'      : show_price,
				#'num'        : min(i[3],skus[i[0]][7]) if skus[i[0]][7]>0 else i[3], # 限购
				'num'        : max(int(i[3]),0),
				'promote'    : skus[i[0]][6],
				'promote_img': app_helper.left_tag['default'],
				'image'      : '/%s/%s' % (skus[i[0]][1][:2], skus[i[0]][1]),
				'max_can_buy': max(int(i[3]),0) if int(skus[i[0]][7])==0 else int(skus[i[0]][7]),
				'message'    : '' if int(skus[i[0]][7])==0 else '该商品每单限购%d件' % int(skus[i[0]][7])
			}
			if skus[i[0]][3]==1 and float(skus[i[0]][4])<float(i[1]): # 优惠价格比门店价格低
				# 调整价格显示，有分的金额才显示2位小数， 2015-10-30
				if ('%.2f'%float(skus[i[0]][4]))[-1]=='0':
					new_one['sale_price'] = ['%.1f' % float(skus[i[0]][4])]
				else:
					new_one['sale_price'] = ['%.2f' % float(skus[i[0]][4])]

			# 设置标签 2015-09-27
			if new_one['promote']>0:
				for j in app_helper.left_tag['tags']:
					if i[2] in j['skus']:
						new_one['promote_img'] = j['tag']
						break

			if version=='v3':
				new_one['max_can_buy'] = int(skus[i[0]][7]) # 0 表示不限购
				# 设置 买X送Y
				if i[2] in app_helper.buy_X_give_Y.keys():
					new_one['xx'] = app_helper.buy_X_give_Y[i[2]][0] 
					new_one['yy'] = app_helper.buy_X_give_Y[i[2]][1]
				else:
					new_one['xx'] = 100000
					new_one['yy'] = 0

			data.append(new_one)

		#print data

		# 返回最近shop
		if version=='v2':
			return json.dumps({
				'ret' : 0, 
				'data' : {
					'total'      : len(data), 
					'page_size'  : int(param.page_size),
					'page_index' : int(param.page_index),
					'skus'       : data,
				}
			})
		elif version=='v3':
			return json.dumps({
				'ret' : 0, 
				'data' : {
					'total'      : len(data), 
					'page_size'  : int(param.page_size),
					'page_index' : int(param.page_index),
					'skus'       : data,
					'only_one'   : app_helper.only_one,
					'only_one_message' : '每单只能购买1种该促销活动商品'
				}
			})

